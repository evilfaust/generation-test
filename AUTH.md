...
│  • students и персональные данные — закрыты (v3.9.120)  │
│  • tasks/works и прочий контент — читаются анонимно     │
└─────────────────────────────────────────────────────────┘
```

**Состояние уровня 4 (проверено 23.08.2026 запросом к проду):**

- `students` — закрыты, анонимный запрос возвращает пустой список (мультиучительство, этап 4).
- `tasks`, `works` и остальной контент — **читаются анонимно**: `GET /api/collections/tasks/records`
  отдаёт записи без токена. Для контента это сознательная договорённость приватной платформы,
  но помнить об этом надо: viewer может через DevTools получить то, что UI ему не показывает.
- Правила ~35 коллекций переписаны миграцией `1783300000_multiteacher_rules.js` (down = снапшот).

## Мультиучительство (v3.9.117–120, в проде с 03.07.2026)

Данные учителей изолированы друг от друга — авторизация теперь не только про роли,
но и про владельца.

- **`owner`** (relation → `teachers`) добавлен в 15 коллекций миграцией `1783100000`,
  существующие записи забэкфилены на `evilfaust`.
- Фронтенд: `withOwner()` проставляет владельца при создании, `andOwner()` подмешивает
  фильтр в списки (`shared/services/pb/client.js`). Superadmin видит всё.
- Ученики привязываются к учителю при добавлении в группу (claim); аналитика скоупится
  по `student.owner`. Этап 5 — передача учеников между учителями и массовое создание
  ученических аккаунтов в карточке группы.
- **`ai_enabled`** (миграция `1783200000`) — тумблер «ИИ» в `UserManager`. Гейтит
  LLM-кнопки в UI (`useAuth().aiEnabled`) и запросы к backend helper: `/latex-fix`
  и `/scan-blank` возвращают 401 без токена учителя (env `REQUIRE_TEACHER_AI_AUTH=1`).
- ⚠️ Грабля деплоя: боевой backend helper лежит в `/opt/pocketbase/pdf-service/pdf-service.js`
  (подпапка!), а не в корне `/opt/pocketbase/`.
...
- username: `evilfaust`
- password: задан в миграции, **должен быть сменён администратором после первого логина**
  (см. UserManager → редактирование пользователя)
