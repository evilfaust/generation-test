...
const CORE_MODULES = [
  ...
  },
  {
    ...
    desc: 'Случайная выборка задач по критериям с мгновенной печатью. Подходит для карточек, маршрутных листов и других коротких печатных форматов.',
    details: [
  ...
  },
  {
    ...
    desc: 'Распределение задач по тегам или сложности с точным контролем количества. Быстрое создание устного счёта, карточек и маршрутных листов.',
    details: [
  ...
  },
  {
    ...
    desc: 'Мощный конструктор структуры с блоками задач из разных тем. Независимые фильтры для каждого блока. Три режима генерации: уникальные задачи / одинаковые с перемешиванием / одинаковые. Подходит и для печатных worksheets с нестандартной подачей.',
    details: [
  ...
  },
  {
    ...
    details: [
      'Сохранение состояния .ggb + экспорт PNG с обрезкой и конвертацией в SVG',
      'Расширенный редактор условия/решения (CodeMirror + KaTeX), 5 уровней сложности',
      'Фото-вложения к решению вместо набора LaTeX (загрузка в библиотеку материалов)',
      'Печатные листы A4/A5 с гибкой раскладкой, тумблером клетки и размером текста S/M/L',
      'ТДФ — конспекты Теорем, Определений, Формул (8 типов, включая геометрическую формулу с двумя чертежами)',
      'Печать ТДФ A4: эталонный конспект + бланки вариантов для учеников',
    ],
    tags: ['GeoGebra', 'SVG/PNG', '5 уровней', 'Фото-решения', 'ТДФ'],
  },
  {
    num: '07', color: 'pink',
    title: 'Модуль теории',
    ...
    details: [
      'CodeMirror 6 с тулбаром: заголовки, списки, таблицы, палитра формул, загрузка картинок с кадрированием',
      'Единый чистый печатный стиль, превью «вписать по ширине», поля на каждой странице',
      'Каллауты «определение / теорема / пример / ответ / ЧТД», размер картинок S/M/L/XL',
      'Числовые прямые со штриховкой, GeoGebra-чертежи (PNG), статус сохранения и автосейв',
      'Конструктор конспектов для печати с выбором статей',
    ],
    tags: ['CodeMirror', 'Каллауты', 'Числовые прямые', 'Печать конспектов'],
  },
  {
  ...
  },
  {
    ...
    details: [
      'Классы и группы с привязкой учеников, в т.ч. ученики «без аккаунта» для разовых занятий; «Сегодня» — уроки и ближайшие дедлайны',
      'Журнал оценок (ученик × выдача) + журнал внешних работ «Решу ЕГЭ»; посещаемость на уроке и лента по ученику',
      'Тепловые карты ошибок по темам с точечным разбором и кнопкой «работа над ошибками»',
      'КТП с экспортом в Word и печатью, календарь уроков с расписанием пар/интенсивов и drag-переносом дат',
      'Заметки уроков на блочном редакторе с LaTeX (ученики группы, заметки в карточке ученика), библиотека материалов с папками, дедлайны и проходной балл',
    ],
    tags: ['Классы', 'Журнал', 'Посещаемость', 'КТП → Word', 'Календарь', 'Библиотека'],
  },
]

const TEACHER_FEATURES = [
  'Пространство учителя: классы/группы, журнал, посещаемость, КТП, календарь, заметки',
  'Посещаемость уроков (присутствовал/опоздал/уважительная/отсутствовал) и лента по ученику',
  'Библиотека материалов с папками: учебники, методички и файлы для уроков и заметок',
  ...
  'Аналитика: распределение задач по темам, тегам, сложности; честная успеваемость по темам',
...
const DB_COLLECTIONS = [
  ...
  { group: 'Пространство учителя', items: [
    ...
    { name: 'lessons', desc: 'Уроки (календарь)', fields: 'date, title, time_slot, materials (json)', rel: 'teaching_groups' },
    { name: 'lesson_attendance', desc: 'Посещаемость уроков', fields: 'status (present/late/excused/absent)', rel: 'lessons, students' },
    { name: 'teacher_notes', desc: 'Заметки (BlockNote)', fields: 'body (json), note_date, tags, links, is_pinned', rel: 'teaching_groups, lessons' },
    { name: 'materials', desc: 'Библиотека материалов (pb-files)', fields: 'title, category, file, folder', rel: 'folders' },
...
const API_GROUPS = [
  ...
  { name: 'Workspace', count: 18, desc: 'groups, lessons, attendance, notes, ktp, materials library, grade journal + external «Решу» journal & heatmaps' },
...
const VERSION_MILESTONES = [
  ...
  { ver: '3.9.96', date: 'Июнь 2026', title: 'Организация урока', desc: 'Библиотека материалов с папками, расписание пар и интенсивов в календаре, посещаемость уроков, заметки уроков с учениками группы, ученики «без аккаунта», числовые прямые, честная успеваемость по темам', color: 'pink' },
]
...
const TEACHER_NAV = [
  { key: 'workspace', label: 'Моё пространство', comp: 'Сегодня / Группы / Журнал / Посещаемость / КТП / Календарь / Заметки / Библиотека' },
      ...
      <header className="v2-how-hero" id="overview">
        ...
        <div className="v2-container" style={{ position: 'relative', zIndex: 2 }}>
          ...
          <p className="v2-how-subtitle">
            Веб-платформа для учителя математики: банк задач с импортом из «Решу ЕГЭ», 20+ генераторов работ и полные варианты КИМ, семантический поиск похожих задач, модуль геометрии с GeoGebra, ТДФ-конспекты, онлайн-тестирование с достижениями и рабочее пространство учителя — классы, журнал, посещаемость, КТП, календарь с расписанием пар, заметки и библиотека материалов. Три изолированных интерфейса — учитель, ученик и промо-лендинг.
          </p>
