 ...
 *
 * Зачем: GFM-таблица умеет только «сетку с линиями». В КИМ же нужны два
 * других вида таблиц:
 ...
 *
 * Модификаторы (можно перечислять через запятую: `{без линий, без шапки}`):
  ...
  for (let i = kids.length - 1; i >= 0; i -= 1) {
    ...
    if (child.type === 'table') {
      ...
      if (prev && prev.type === 'paragraph') {
        mods = parseTableDirective(nodeText(prev));
        // Директиву-параграф убираем из дерева; идём с конца, поэтому после
        // удаления соседа слева индекс надо подвинуть — иначе на следующем
        // шаге снова попадём на эту же таблицу (уже без директивы).
        if (mods) { kids.splice(i - 1, 1); i -= 1; }
      }
