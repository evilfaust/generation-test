...
**Статус: ✅ РЕАЛИЗОВАНО** в v3.9.31 (27 мая 2026).
Актуальную документацию по парсеру смотри:
- `pocketbase/CLAUDE.md` § sdamgia parser — полное описание серверной части
- `memory/sdamgia_parser.md` — заметка для будущих сессий, грабли, метрики
- Корневой `CLAUDE.MD` § sdamgia parser — обзор pipeline

Документ ниже сохранён как **исторический план** — он был согласован до
начала работ и отражает изначальный дизайн. Реальная имплементация
местами отличается (например: дубли формул дедуплицируются на уровне alt,
а не плейсхолдеров; `![image](url)` оставлено в md с динамической подменой
вместо плейсхолдеров `[[img:role:N]]`).
